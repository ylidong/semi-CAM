...
Get.12mks2<-function(mat_in,num.cell,f.cutoff,sh.cutoff){
  ...
  ##get the second highest expression values##
  #sec.high.ind=apply(mat.rank,2,function(xx) which(xx==(num.cell-1)))
  sec.high=apply(mat_in,2,function(x) x[rank(x,ties.method="random")==(num.cell-1)])
  fold=high/sec.high ##fold high/sec.high
...
GET.newmarkers2<-function(est.cse_in,num.cell,f.cutoff,sh.cutoff){
  #est.cse_in=A_cse;num.cell=ncell;f.cutoff;sh.cutoff
  #est.cse_in=A_cse;num.cell=4
  #est.cse_in=t(X.initial[!rownames(X.initial)%in%unlist(MKS.known),]);num.cell=ncell;f.cutoff;sh.cutoff
  ###markers only expressed in one cell type###
...
umm_mks_NMF.inout.new<-function(data,P.initial,X.initial,MKS.known,Markers.ls_initial,maxInter,ncell,NMF.method,NMF.run,criterion="MSE.Y",f.cutoff,sh.cutoff){
  
  #data=data.mix.filter;P.initial=prop.est;MKS.known=NULL;Markers.ls_initial= CAM.org.out$V.pick;maxInter=10;ncell=ncell;NMF.method="ssKL";criterion="MSE.Y"
  #data=data.mix.filter;P.initial=ssCAM$P.KL;MKS.known=MKS.initial;Markers.ls_initial=ssCAM$MKS;maxInter=10;NMF.method="ssKL" 
  #data=data_est;P.initial=P.NMF;MKS.known=mks_in;Markers.ls_initial=ssCAM$V.pick;criterion="MSE.Y"
  
  #data=data_est;P.initial=P.NMF;X.initial=X.NMF;MKS.known=mks_in;Markers.ls_initial=MKS.order;maxInter;ncell;NMF.method;NMF.run;criterion="MSE.Y";f.cutoff;sh.cutoff
  ...
  while(inter<maxInter){
    ...
    Markers.ls_update=GET.newmarkers2(est.cse_in=t(X.initial[!rownames(X.initial)%in%unlist(MKS.known),]),num.cell=ncell,f.cutoff,sh.cutoff)
...
if(F){
umm_mks_NMF.inout.new.MKS<-function(data,P.initial,X.initial,MKS.known,Markers.ls_initial,maxInter,ncell,NMF.method,NMF.run,criterion="MSE.Y",f.cutoff,sh.cutoff){
...
umm_mks_NMF.onlyadd.new<-function(data,P.initial,X.initial,MKS.known,Markers.ls_initial,maxInter,ncell,NMF.method,NMF.run,criterion="MSE.Y",f.cutoff,sh.cutoff){
  ...
  while(inter<maxInter){
    ...
    Markers.ls_add=GET.newmarkers2(est.cse_in=t(X.initial[!rownames(X.initial)%in%unlist(Markers.ls_initial),]),num.cell=ncell,f.cutoff,sh.cutoff)
    Markers.ls_update=mapply(union,Markers.ls_add,Markers.ls_initial,SIMPLIFY=FALSE)
