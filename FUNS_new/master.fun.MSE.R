master.fun.MSE<-function(data.mix.use_in,MKS.list.use_in,P.use_in){
  ##################
  ###CAM original###
  ##################
  ##avg estimate##
  ...
  CAM.AE=MSE.fun(P.est=CAM.cor$P.order,P.use)
  ...
  RES.CAM=data.frame(method="CAM",known.vertice=0,known.markers.pct=0,vertice.select=0,COR.KL=CAM.cor$COR,COR.Fro=CAM.cor$COR,AE.KL=CAM.AE,AE.Fro=CAM.AE)
  ...
  CAM.KL.AE=MSE.fun(P.est=CAM.KL$P.order,P.use)
  CAM.Fro.AE=MSE.fun(P.est=CAM.Fro$P.order,P.use)
  
  RES.CAM.NMF=data.frame(method="CAM.NMF",known.vertice=0,known.markers.pct=0,vertice.select=0,COR.KL=CAM.KL$COR,COR.Fro=CAM.Fro$COR,AE.KL=CAM.KL.AE,AE.Fro=CAM.Fro.AE)
  ...
  for(known.vertice in 1:ncell){
    ...
    for(j in 1:ncol(vertice.select)){
      ...
      ssCAM.KL.AE=MSE.fun(P.est=ssCAM.KL$P.order,P.use)
      ssCAM.Fro.AE=MSE.fun(P.est=ssCAM.KL$P.order,P.use)
      
      #res.ssCAM.list[[num.run]]=list("vertice.select"=paste0(vertice.select),"ssCAM"=ssCAM.res)
      out=data.frame(method="ssCAM",known.vertice=known.vertice,known.markers.pct=known.markers.pct,vertice.select=paste0(as.vector(vertice.select[,j]),collapse = ""),
                     COR.KL=ssCAM.KL$COR,COR.Fro=ssCAM.Fro$COR,AE.KL=ssCAM.KL.AE,AE.Fro=ssCAM.Fro.AE)
  ...
  NMF.KL.AE=MSE.fun(P.est=NMF.COR.KL$P.order,P.use)
  NMF.Fro.AE=MSE.fun(P.est=NMF.COR.Fro$P.order,P.use)
  
  res.NMF.list=list("NMF"=NMF.res)
  RES.NMF=data.frame(method="NMF",known.vertice=ncell,known.markers.pct=known.markers.pct,vertice.select="123",
                     COR.KL=NMF.COR.KL$COR,COR.Fro=NMF.COR.Fro$COR,AE.KL=NMF.KL.AE,AE.Fro=NMF.Fro.AE)
  
  ##DSA##
  DSA.res= ged(as.matrix(data.mix.use), MarkerList(MKS.list.use), "DSA")
  P.DSA=DSA.res@fit@H
  DSA.COR=match.cor(P.est=P.DSA,P.use)
  DSA.AE=MSE.fun(P.est=DSA.COR$P.order,P.use)
  RES.DSA=data.frame(method="DSA",known.vertice=ncell,known.markers.pct=known.markers.pct,vertice.select="123",
                     COR.KL=DSA.COR$COR,COR.Fro=DSA.COR$COR,AE.KL=DSA.AE,AE.Fro=DSA.AE)
